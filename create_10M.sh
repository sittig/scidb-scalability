...
SEL_VALUE_INTO_TEST="
  SELECT value
    ...
    FROM between ( CRV_DATA, 0, 0, 999999, null )
  "
time iquery -nq "${SEL_VALUE_INTO_TEST}"
##
#SET LANG AQL;
#SET FETCH;
time iquery -q "SELECT * FROM op_now()"
...
#SET LANG AQL;
#SET FETCH;
time iquery -q "SELECT COUNT(*) FROM CRV_DATA_10M_TEST"
time iquery -q "SELECT MAX ( row_num ) FROM CRV_DATA_10M_TEST"
##
    ...
    cast ( 
      substitute ( 
        ( SELECT MIN ( value ) AS value 
            FROM  ( SELECT row_num + ( 1 * 1000000 ) as new_row_num,
...
INSERT_INTO_CRV_DATA="
   insert ( 
    cast ( 
      substitute ( 
        ( SELECT MIN ( value ) AS value 
            FROM  ( SELECT row_num + ( 2 * 1000000 ) as new_row_num,
                           attr_num AS new_attr_num,
                           value
                      FROM between ( CRV_DATA_10M_TEST, 0, 0, null, null )
                  )
...
INSERT_INTO_CRV_DATA="
   insert ( 
    cast ( 
      substitute ( 
        ( SELECT MIN ( value ) AS value 
            FROM  ( SELECT row_num + ( 4 * 1000000 ) as new_row_num,
                           attr_num AS new_attr_num,
                           value
                      FROM between ( CRV_DATA_10M_TEST, 0, 0, null, null )
                  )
...
INSERT_INTO_CRV_DATA="
   insert ( 
    cast ( 
      substitute ( 
        ( SELECT MIN ( value ) AS value 
            FROM  ( SELECT row_num + ( 8 * 250000 ) as new_row_num,
                           attr_num AS new_attr_num,
                           value
                      FROM between ( CRV_DATA_10M_TEST, 0, 0, null, null )
                  )
...
##
##  Rename the CRV_DATA_10M_TEST to CRV_DATA_1M
#
##### EVENTUALLY change back but
# renaming some data so we can test iteratively!
# CRV_DATA --> CRV_DATA_100K
# CRV_DATA_10M_TEST --> CRV_DATA
# Then we can run the queries again on the larger dataset!
#time iquery -q "RENAME ARRAY CRV_DATA_10M_TEST TO CRV_DATA_1M"
# time iquery -q "DROP ARRAY CRV_DATA_100K"
time iquery -q "DROP ARRAY CRV_DATA_1M"
time iquery -q "RENAME ARRAY CRV_DATA TO CRV_DATA_1M"
time iquery -q "DROP ARRAY CRV_DATA"
time iquery -q "RENAME ARRAY CRV_DATA_10M_TEST TO CRV_DATA"
##
